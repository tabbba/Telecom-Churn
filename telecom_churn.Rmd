...
ggplot(churn_percent, aes(x = "", y = Percent, fill = Churn)) +
  ...
  theme_void() +
  scale_fill_manual(values = c("False" = "#15aabf", "True" = "#ff8787"), labels = c("False" = "No", "True" = "Yes")) +
...
ggplot(long_data, aes(x = Value, fill = Value)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("No" = "#15aabf", "Yes" = "#ff8787"), labels = c("False" = "No", "True" = "Yes")) +
  theme(
...
```{r message=FALSE, warning=FALSE, echo=FALSE, fig.align='center', fig.show='hold'}
...
The correlation matrix shows that there is a **perfect correlation between the charge variables and minutes variables** so we can drop all the minutes one: **Total.day.minutes, Total.eve.minutes, Total.night.minutes, Total.intl.minutes**.
...
ggplot(churn_percent, aes(x = Customer.service.calls, y = Percent, fill = Churn)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("False" = "#ADD8E6", "True" = "#dab0ff"), labels = c("False" = "No", "True" = "Yes")) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
In proceeding our analysis we should first deal with the 51 states: we can group them by region; mid west, north east, south east, south west, west.
...
We oversample the minority class to balance the dataset, and as a result now we have 4260 total observations.
...
It is finally time to start the analysis with the full model. We'll begin by employing a logistic regression model as our baseline. This approach is straightforward and effective for predicting binary outcomes, which makes it an ideal starting point. By using logistic regression as our benchmark, we can clearly assess the performance improvements offered by more complex models. As we proceed, we'll compare this baseline model's accuracy against more sophisticated algorithms to determine whether the increased complexity justifies the potential gains in prediction accuracy. This step-by-step comparison helps us identify which models enhance our analytical capabilities and which may not be worth the additional complexity.
...
We decided to leverage both LASSO and Ridge regularization methods to fine-tune our logistic regression model, aiming to enhance its predictive capabilities while addressing potential multicollinearity and feature redundancy.
...
Using the Ridge regularization technique with the glmnet method and cross-validation, we achieved a maximum accuracy of 77.07%. The best-performing parameters selected are alpha = 0 and lambda = 0.01034, indicating Ridge regularization almost without penalty.
...
As shown in the above plots, the AUC for the Lasso model is 0.778, while the AUC for the Ridge model is 0.776. Both models demonstrate good discrimination ability, with the Lasso model slightly outperforming the Ridge model.
...
The above plots illustrate how the accuracy of logistic regression models changes with different lambda values for Lasso and Ridge regularization techniques. In the Lasso plot, accuracy starts high at 0.766 and gradually decreases to 0.723 as lambda increases. Meanwhile, the Ridge plot shows a more consistent accuracy around 0.77, with minor fluctuations, indicating stable performance across various penalty strengths.
...
Akaike stands out with the highest accuracy at approximately 83.69% and also shows the best overall metric results, indicating strong model performance across multiple evaluation criteria. Other models, such as Bayesian and Full Logistic, offer competitive but slightly lower scores in accuracy and other metrics. Logistic Ridge and Logistic Lasso have notably lower accuracies but higher precision values, which might make them preferable in scenarios where the cost of false positives is high. 
...
After pruning, the resulting tree is the following (there are some small differences with respect to the previous tree):
...
These values inside the contingency table show that the pruned tree model are exactly the same as the full tree model, even if the structure of the tree changed, which is a good sign of the model's robustness.
...
In this section, we evaluate our bagged model's performance on the validation dataset. Our aim with this plot is to illustrate the importance of each predictor variable in the random forest model. At the top of our results we find 'Customer service calls', 'International plan', 'Total international calls'.
...
The vertical line highlights the number of trees that minimizes the validation error, helping identify the optimal number of trees for the model. In our specific case the value is 35.
